...
public class OpenAIProvider implements AIProvider {
    ...
    public OpenAIProvider(
            ...
            @Value("${ai.model.openai:gpt-4}") String model,
            ...
            @Value("${codesage.ai.openai.input-cost-per-million-tokens:0}") double inputCostPerMillionTokens,
            @Value("${codesage.ai.openai.output-cost-per-million-tokens:0}") double outputCostPerMillionTokens) {
        this(
                webClientBuilder.baseUrl(baseUrl).build(),
                new RemoteAIReviewParser(objectMapper),
                circuitBreakerRegistry.circuitBreaker("codesage.openai"),
                apiKey,
                model,
                timeout,
                maxDiffChars,
    ...
    public AnalysisResult analyze(String codeDiff) {
        ...
        long inputTokens = response.path("usage").path("prompt_tokens").asLong(estimateTokens(codeDiff));
        long outputTokens = response.path("usage").path("completion_tokens").asLong(estimateTokens(content));
        ...
        Mono<JsonNode> request = webClient.post()
                ...
                .bodyValue(Map.of(
                        ...
                        "messages", List.of(
                                ...
                                Map.of("role", "user", "content", userPrompt(codeDiff)))))
                .retrieve()
